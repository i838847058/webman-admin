...
<div id="has-many-{{$column}}" class="nav-tabs-custom has-many-{{$column}}">
    ...
    <template class="nav-tab-tpl">
        <li class="new">
            <a href="#{{ $relationName . '_new_' . \Shx\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}" data-toggle="tab">
                &nbsp;New {{ \Shx\Admin\Form\NestedForm::DEFAULT_KEY_NAME }} <i
    ...
    <template class="pane-tpl">
        <div class="tab-pane fields-group new"
             id="{{ $relationName . '_new_' . \Shx\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}">
            {!! $template !!}
        </div>
    </template>
